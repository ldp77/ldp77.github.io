    ...
    def add_evidence(self, all_x, all_y):
        ...
        def build_tree(data_x, data_y):
            ...
            for col_idx in range(data_x.shape[-1]):
                correlations = np.append(correlations, np.corrcoef(data_x[:, col_idx], data_y)[0,1])

            strongest_feature_idx = np.argmax(abs(correlations))
        ...
        def build_tree(data_x, data_y):
            ...
            if self.verbose:
                print("NEW CALL - X: {}, Y: {}".format(data_x, data_y))

            if len(data_y) == 1:

                if self.verbose:
                    print("Returning at len(y) == 1")

                return np.array([-1, data_y[0], -1, -1], dtype=np.float64)
            if np.all(data_y == data_y[0]):

                if self.verbose:
                    print("Returning at all y's equal")
                    print(data_y)
            ...
            # Sort and split data according to strongest feature
            left_idx, right_idx = [], []
            for i in range(len(data_x)):
                if data_x[i][strongest_feature_idx] <= splitval:
                    left_idx.append(i)
                else:
                    right_idx.append(i)

            # Handle edge case where ALL Data is <= median
            if len(right_idx) == 0:
                left_idx, right_idx = [], []
                for i in range(len(data_x)):
                    if data_x[i][strongest_feature_idx] < splitval:
            ...
            left_x, right_x = data_x[left_idx], data_x[right_idx]
            left_y, right_y = data_y[left_idx], data_y[right_idx]

            left_tree = build_tree(left_x, left_y)
            right_tree = build_tree(right_x, right_y)

            root = np.array([strongest_feature_idx, splitval, 1, left_tree.shape[0]+1], dtype=np.float64)
